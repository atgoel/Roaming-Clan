 ...
 * @since 1.6.0
 * @param int     $product_id The product ID.
 * @param boolean $children   No needed parameter.
 */
 ...
 *
 * @since 1.6.10
 * @param WC_Product_Variable $product Then variable product to sync.
 */
 ...
 * @return boolean
 */
function wcpbc_is_admin_ajax() {
	$is_admin_ajax = false;

	if ( defined( 'DOING_AJAX' ) && DOING_AJAX && isset( $_POST['action'] ) && is_string( $_POST['action'] ) ) {
		$ajax_action        = sanitize_text_field( $_POST['action'] );
		$admin_ajax_actions = array( 'woocommerce_add_variation', 'woocommerce_load_variations', 'woocommerce_save_variations', 'woocommerce_bulk_edit_variations', 'inline-save', 'woocommerce_add_order_item', 'wcpbc_hide_notice' );
		$is_admin_ajax      = in_array( $ajax_action, $admin_ajax_actions ) || apply_filters( 'wc_price_based_country_is_admin_ajax_action', false, $ajax_action );
	}

	return $is_admin_ajax;
}
