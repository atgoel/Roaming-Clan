...
function bp_customer_members( $args = '' ) {
    global $wpdb;
	global $members_template;
		...
		"
                ...
                WHERE p.post_status IN ( 'wc-cancelled', 'wc-on-hold', 'wc-completed','wc-processing' )
                AND pm.meta_key IN ( '_customer_user' )
                AND im.meta_key IN ( '_product_id' )
                AND im.meta_value = " . $page_id                 
		);
       if($result)
       {
	// Default user ID.
	...
	$r = bp_parse_args( $args, array(
		...
		'include'             => $result,    // Pass a user_id or a list (comma-separated or array) of user_ids to only show these users.
		'exclude'             => false,    // Pass a user_id or a list (comma-separated or array) of user_ids to exclude these users.
